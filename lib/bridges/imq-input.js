...
class IronMQInput {
  ...
  constructor(config, logger, imq){
    ...
    const cronTime = config.options.imq.cronTime || '*/30 * * * * *';
  ...
  createDispatchTask(message){
    return (done) => {
      ...
      try {
        ...
        this.logger.log(['warn'], { message: 'Could not parse body of message.', error: e });
      }
      this.dispatch(body, (err) => {
        if (err){
  ...
  tick(){
    ...
    this.queue.reserve({ n: this.messagesPerRequest, timeout: this.timeout }, (err, messages) => {
      ...
      // IronMQ will apparently give you nothing if there are no messages.
      messages = messages || [];
      const tasks = [];
