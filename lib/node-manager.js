...
class NodeManager {
  ...
  dispatchChildMessage(packet){
    this.logger.log(['info'], { message: 'Received message from a Node.', data: packet });
  }
  ...
  getLaunchConfig(nodeConfig){
    ...
    launchConfig.name = NodeManager.getWorkGroupName(launchConfig);
    launchConfig.args = [ 'run', NodeManager.encode(nodeConfig) ];
  ...
  spawnNode(nodeConfig, callback){
    ...
    pm2.start(launchConfig, (err, apps) => {
      ...
      const processId = NodeManager.getProcessId(apps, launchConfig.name);
